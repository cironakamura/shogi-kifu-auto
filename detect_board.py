...
def detect_shogi_grid_and_crop(image_path, output_dir="output_cells_grid"):
    ...
    lines = cv2.HoughLinesP(
        warped_edges, 1, np.pi / 180, threshold=200, minLineLength=100, maxLineGap=20
    )
    if lines is None:
    ...
    if len(v_lines) < 10 or len(h_lines) < 10:
        print(
            f"十分な格子線が見つかりませんでした。(垂直: {len(v_lines)}本, 水平: {len(h_lines)}本)"
        )
    ...
    # 確認用に検出した線が描画された画像を保存
    detected_lines_filename = os.path.join(output_dir, "detected_lines.png")
    cv2.imwrite(detected_lines_filename, warped)
    print(f"検出した線が描画された画像を '{detected_lines_filename}' に保存しました。")


if __name__ == "__main__":
    input_image_file = "shogi_board_5.png"  # 入力画像ファイルを指定
